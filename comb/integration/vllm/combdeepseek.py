...
from transformers import DeepseekV2Config, AutoTokenizer, BatchFeature
import typing
...
from vllm.model_executor.models.deepseek_v2 import (
    DeepseekV2MLP, DeepseekV2MoE, DeepseekV2DecoderLayer, DeepseekV2MixtureOfExperts
)
...
from comb.integration.hf.CombDeepSeek import CombDeepSeekConfig
...
class CombDeepseekCrossAttentionDecoderLayer(nn.Module):
...
class CombDeepseekTextModel(nn.Module):
    ...
    def __init__(self,
        ...
        # NOTE: We modify the hf_config to initialize `DeepseekV2DecoderLayer`
        vllm_config.model_config.hf_config = config  # type: ignore
        self.layers = nn.ModuleList([
            DeepseekV2DecoderLayer(vllm_config=vllm_config, prefix=f"{prefix}.layers.{idx}")
...
class CombDeepseekForCausalLM(nn.Module, DeepseekV2MixtureOfExperts, SupportsLoRA):
    base_model_prefix = "language_model"
    _no_split_modules = [
        "CombDeepseekCrossAttentionDecoderLayer", "DeepseekV2DecoderLayer"
    ]
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.model = CombDeepseekTextModel(vllm_config=vllm_config,
...
class CombProcessingInfo(BaseProcessingInfo):
    def get_hf_config(self) -> CombDeepSeekConfig:
        return self.ctx.get_hf_config(CombDeepSeekConfig)
...
class CombDeepseekForConditionalGeneration(nn.Module, SupportsMultiModal):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        super().__init__()
        config: CombDeepSeekConfig = vllm_config.model_config.hf_config
        quant_config = vllm_config.quant_config
        self.config = config
        self.quant_config = quant_config
        ...
        self.language_model = CombDeepseekForCausalLM(
